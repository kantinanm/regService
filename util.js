...
exports.findSubjectEnrollement = function (student_id, acadyear, semester, cb) {
  var config = {
    student_id: student_id,
    acadyear: acadyear,
    semester: semester
              ...
              url_hock(cookie, linkGetStudentInfo, function (tag) {
                ...
                var reqAgain = http.request(options_call, function (response) {
                  toUTF8(response, function (utf8strInner) {
                    ...
                    getLink(utf8strInner, function (links) {
                      for (var i = 0; i < links.length; i++) {
                        if ((/^enroll_result/).test(links[i].href)) {
                          let urlCheckEnroll = `enroll_result.asp?acadyear=${config.acadyear}&maxsemester=3&semester=${config.semester}`
                          query_enroll(cookie, urlCheckEnroll, function (enrolls) {

                            student_info['enroll'] = enrolls;
        ...
        toUTF8(res, function (utf8str) {
          htmlToJson.parse(utf8str, {
            ...
            for (let i = 0; i < result.tr.length; i++) {
              ...
              if (result.tr[i].text == "จำนวนหน่วยกิตรวม") {
                lastIndex = i - 1;
                total_unit = result.tr[i + 1].text.trim();
              }
            }
            ...
            for (let j = 0; j < (lastIndex - 24) / 5; j++) {
              console.log("start new row " + j);
              console.log(step);
            ...
            for (let n = 0; n < indexKeeper.length; n++) {
              console.log("read row " + n);

              //let temStr = result.tag[indexKeeper[n]['section']].text.split(' ');

              var tmp = {
                'subject_id': result.tr[indexKeeper[n]['subject_id']].text.trim(),
                'subject_name': result.tr[indexKeeper[n]['subject_name']].text.trim(),
                'type': result.tr[indexKeeper[n]['type']].text.trim(),
                'unit': result.tr[indexKeeper[n]['unit']].text.trim(),
                'section': result.tr[indexKeeper[n]['section']].text.trim(),
              }
